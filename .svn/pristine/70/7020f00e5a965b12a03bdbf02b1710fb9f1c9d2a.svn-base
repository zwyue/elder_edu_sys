        ...
        layer.open({
            ...
            yes: function (index, layero) {//该回调携带两个参数，分别为当前层索引、当前层DOM对象
                ...
                if (MakeupClassRoom == '请选择') {
                    layui.use('layer', function () {
                        var layer = layui.layer;
                        layer.alert('请选择补课教室', {area: ['200px', '180px']});
                    });
                    return false;
                }
                //最后关闭弹出层
    ...
    function btnXz() {
        ...
        if (qjLeaveName === '' || qjLeaveDate === '' || qjLeaveClass === '' || qjMakeupDate === '' || qjMakeupClass === '') {
            layui.use('layer', function () {
                var layer = layui.layer;
                layer.msg('文本框不能为空！', {time: 1000, area: ['180px', '50px']});
            })
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    setTimeout(function () {
                        window.location.href = "jxgl_qj.html";
                    }, 2000)
                },
                error: function(error) {
	                layui.use('layer', function () {
