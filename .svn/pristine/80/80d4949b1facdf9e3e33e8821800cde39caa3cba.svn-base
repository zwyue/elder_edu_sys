...
标题<textarea name="title">${classsuggest.title}</textarea>
...
<div>班级
    <select name="classes" id="classes">
        <option value="${classsuggest.classid}">${classsuggest.classname}</option>
    </select>
    <input type="hidden" name="classid" id="classid">
    <input type="hidden" name="classname" id="classname">
</div>
<div>
    回复情况：<textarea name="reply" >${classsuggest.reply}</textarea>
</div>
<div>
    建议时间：<input type="date" name="suggesttime" value="${classsuggest.suggesttime}" id="suggesttime">
</div>
...
<script>
    ...
    document.getElementById('btn1').addEventListener('click', function () {
        ...
        $.ajax({
            type:"post",
            url:"/classsuggest/update",
            data:{content:content,title:title,id:${classsuggest.id},classid:classid,classname:classname},
            dataType:"json",
            async:true,
    ...
    editor.txt.html('${classsuggest.content}');
