    	...
    	$.ajax({
    		...
    		async:true,
    		dataType:'json',
    		url:'api/statistics/student/newrate',
    		success:function(data){
	            ...
	            for (var i = 0; i < listData.length; i++){
	                termPeople.push(listData[i].newRate);    //挨个取出学期名字回填到学期姓名数组
	            }
...
//				   		myChart.showLoading();    //数据加载完之前先显示一段简单的loading动画
				  		...
				  		$.ajax({
					  		type:'get',
					  		async:true,
					  		url:'api/statistics/student/newrate?majorId='+callBackData.classMajorId,
							dataType:'json', 		
					  		success:function(data){
				    			('请求成功');
				    			var listData = data.data.statisticInfo;
					            ...
					            for (var i = 0; i < listData.length; i++){
					                termPeopleMajor.push(listData[i].newRate);    //挨个取出学期名字回填到学期姓名数组
					            }
					            myChart.hideLoading();    //隐藏加载动画
					            myChart.setOption({        //加载数据图表
				                xAxis: {
				...
				//				  myChart.setOption(option);				    			
					   ...
					   $.ajax({
					  		type:'get',
					  		async:true,
					  		url:'api/statistics/student/newrate?classId='+callBackData.classesId,
							dataType:'json', 		
					  		success:function(data){	
				  				('请求成功');
				    			var listData = data.data.statisticInfo;
					            ...
					            for (var i = 0; i < listData.length; i++){
					                termPeopleClass.push(listData[i].newRate);    //挨个取出学期名字回填到学期姓名数组
					            }
					            myChart.hideLoading();    //隐藏加载动画
					            myChart.setOption({        //加载数据图表
				                xAxis: {
