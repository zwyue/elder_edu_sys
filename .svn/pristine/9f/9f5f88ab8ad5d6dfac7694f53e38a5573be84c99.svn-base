...
import com.zrtjoa.controller.student.StudentController;
...
import static com.zrtjoa.common.TimeUtil.calAge;
...
public class ClassesServiceImpl implements ClassesService {
    ...
    public List<Map<String,Object>> queryClsCateToAcquireCls() {
        ...
        categories.forEach(category -> {
            Map<String,Object> cateMap = new HashMap<>(MAP_DEFAULT_SIZE);
            //过滤出该分类下的专业
            ...
            profs.forEach(profession -> {
                ...
                Map<String,Object> prfMap = new HashMap<>(MAP_DEFAULT_SIZE);
                //只有班级存在的专业才会放进分类
                if(classes.size()!=0){
                    classes.forEach(c->{
                        Map<String,Object> clsMap = new HashMap<>(MAP_DEFAULT_SIZE);
    ...
    public List<Classes> queryClsByPrfs(String prfIds) {
        //根据专业id查询班级
        return classesDao.getClsByPrfIds(Arrays.asList(prfIds.split(COMMA)));
    }
