...
public class StudentServiceImpl implements StudentService {
    ...
    public Map enterStudentInfo(Student student, StudentEnter studentEnter,String flag) {
        ...
        if(existStudent!=null){
            ...
            map.keySet().forEach(key->{
                ...
                if(stuRoster!=null){
                    newMap.put(String.valueOf(stuRoster.get("stunumber")),stuRoster.get("classname")+COMMA+stuRoster.get("classId"));
                }else {
            ...
            map.keySet().forEach(key->{
                stringBuilder.append(key).append(COMMA);
            });
        }else {
    ...
    @PagingQuery
    @Override
    public Map queryAllStudents(String name, String idCard) {
        logger.info("......查询学生信息.......");
        Map<String,Object> map = new HashMap<>(MAP_DEFAULT_SIZE);
        map.put("name",name);
        ...
        students.forEach(student -> {
            ...
            //查询班级
            List<Classes> classes = classesDao.queryClsByIds(Arrays.asList(student.getClassid().split(COMMA)));
            StringBuilder headMaster = new StringBuilder() , majorName = new StringBuilder() , cateName = new StringBuilder();

            //所以班级不能随便删除，后面修改吧todo
            if(classes.size()!=0) {
                classes.forEach(cls->{
                    headMaster.append(cls.getHeadmastername()).append(COMMA);
                    majorName.append(cls.getMajorname()).append(COMMA);
                    cateName.append(cls.getCatename()).append(COMMA);
                });
                //学生班主任
                student.setHeadMasterName(headMaster.substring(0,headMaster.lastIndexOf(COMMA)));
                //专业、类别
                student.setMajorName(majorName.substring(0,majorName.lastIndexOf(COMMA)));
                student.setCateName(cateName.substring(0,cateName.lastIndexOf(COMMA)));
            }
        });
    ...
    public Map queryStuAllInfo(Integer id,String idCard) {
        ...
        if(studentEnter!=null && StringUtils.isNotBlank(studentEnter.getClassid())){
            Arrays.asList(studentEnter.getClassid().split(COMMA)).forEach(clazzId->{
    ...
    public Map update(StuAndStuEnter stuAndStuEnter,String flag) {
        ...
        if(StringUtils.isNotBlank(stuAndStuEnter.getClassid())&&StringUtils.isNotBlank(stuAndStuEnter.getClassname())){
            clsIds = new String[stuAndStuEnter.getClassid().split(COMMA).length];
            for (int i = 0; i <stuAndStuEnter.getClassid().split(COMMA).length; i++) {
                clsIds[i] = stuAndStuEnter.getClassid().split(COMMA)[i].split(SLASH)[2];
            }
            String originClsIds = studentDao.selectByPrimaryKey(stuAndStuEnter.getId()).getClassid();
            orginClassIds = originClsIds.split(COMMA);
        ...
        x.forEach(x1->{
            if(y.contains(x1)){
                willContain.append(x1).append(COMMA);
            }
            else {
                willDelIds.append(x1).append(COMMA);
            }
        });

        y.forEach(y1->{
            if(!x.contains(y1)){
                willAddIds.append(y1).append(COMMA);
            }
        });
        ...
        if(StringUtils.isNotBlank(willAddIds.toString())||StringUtils.isNotBlank(willDelIds.toString())){
            ...
            if(StringUtils.isNotBlank(willDelIds.toString())){
                ...
                map.put("clsIds",willDelIds.toString().split(COMMA));
            ...
            if(StringUtils.isNotBlank(willContain.toString())){
                Map map = new HashMap();
                map.put("stuId",stuAndStuEnter.getId());
                map.put("clsIds",willContain.toString().split(COMMA));
                stuNumbers = rosterDao.queryStuNumByStuIdAndClsIds(map);
            }
            String studentNumbers = "" ;

            for (String stoNo: stuNumbers) {
                studentNumbers += stoNo + COMMA ;
            }

            if(StringUtils.isNotBlank(willAddIds.toString())){
                Map results = formatStuNum(Arrays.asList(willAddIds.toString().split(COMMA)));
                info.put("map",results.get("map"));
                //插入学生信息的学生编号，以逗号隔开
                addNoToRedis.addAll(Arrays.asList(((String) results.get("string")).split(COMMA)));
