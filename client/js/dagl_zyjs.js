		...
		var tableList = [
		  ...
		  $.each(tableList, function(){
            $('<tr id="'+this.id+'">' +
                ...
                '<td>'+this.name+'</td>' +
                '<td>'+this.time+'</td>' +
                '<td><a href="JavaScript:;" class="bg bgActive del">删除</a><a href="dagl_zyjsxg.html" class="bg change">修改</a><a href="javascript:;" class="bg look">导出</a></td>'+
               '</tr>').appendTo('#listZyjs');                   	
          });
		//删除
		$('.del').click(function(){
          	if (confirm('确定要删除吗')){ 
