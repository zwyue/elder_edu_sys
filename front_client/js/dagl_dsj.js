                            ...
                            $.ajax({
                                ...
                                success: function (data) {
                                    ...
                                    $.each(data.data.list, function (i, item) {
                                        ...
                                        html += '<td>' + item.title + '</td>' + '<td>' + item.times + '</td>';
                                        html += '<td><a onclick="sc_delect(' + item.id + ')" href="JavaScript:;" class="bg bgActive del">删除</a>' +
                                            '<a href="dagl_dsjxg.html?id=' + item.id + '" class="bg change">修改</a>' +
                                            '<a href="dagl_dsjck.html?id=' + item.id + '" class="bg change">查看</a>' +
                                            '<a href="api/memorabilia/export?id=' + item.id + '" class="bg look">导出</a></td>';
                                        html += '</tr>';
                                    })
                                    $("#listDsj").html(html);
                                },
				...
				layui.use('layer', function () {
					layer.confirm('确认要删除吗？', function (index) {
						$.ajax({
							type: "get",
							async: false,
							url: "api/memorabilia/delete?id=" + id,
							success: function (data) {
								if(data.code==1){
                            ...
                            success: function (data) {
                                if (data.data.list.length == 0) {
                                    ...
                                    $.each(data.data.list, function (i, item) {
                                        ...
                                        chaXun += '<td>' + item.title + '</td>' + '<td>' + item.times + '</td>';
